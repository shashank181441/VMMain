...
const createProduct = asyncHandler(async (req, res) => {
    const { name, price, category, machineId } = req.body;
    // TODO: Add a new product to the vending machine
    if (
        [name, price, category, machineId].some((field) => field?.trim() === "")
    ) {
    ...
    const product = await Product.create({
        name, category, machine: machineId, price, image_url: image_url.url
    })
    ...
    return res.status(200).json(
        new ApiResponse(200, product, "Product added succesfully.")
    )
...
const getProducts = asyncHandler(async (req, res) => {
    ...
    const products = await Product.find({ machine: machineId });
...
export {
    createProduct, updateProduct, getProducts, getProductDetails, deleteProduct
}