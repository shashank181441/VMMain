...
const createVendingMachine = asyncHandler(async (req, res) => {
    ...
    const newVending = await VendingMachine.create({location, merchantDetails, user: userId})
    ...
    return res.status(201).json(
        new ApiResponse(200, createdVending, "Vending Machine registered Successfully")
    )
});
...
const updateVendingMachine = asyncHandler(async (req, res) => {
    ...
    const updatedVendingMachine = await VendingMachine.findByIdAndUpdate(machineId, {
        location
    }, {new: true})
    ...
    return res.status(201).json(
        new ApiResponse(200, updatedVendingMachine, "Vending Machine updated successfully.")
    )
});
...
const getVendingMachinesByOwner = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, machineByOwner, "The vending machines owned by user is fetched successfully")
    )
});
...
const getVendingMachineDetails = asyncHandler(async (req, res) => {
    ...
    const vending = await VendingMachine.findById(machineId).select(
        "-merchantDetails"
    )
    ...
    return res.status(200).json(
        new ApiResponse(200, vending, "The vending machine details fetched successfully")
    )
});
...
const addMaintenanceRecord = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, maintainedMachine, "The vending machine was updated successfully." ));
});
...
const getMaintenanceRecords = asyncHandler(async (req, res) => {
    ...
    const vendingRecords = await VendingMachine.findById(machineId).select(
        "-merchantDetails"
    )
    ...
    return res.status(200).json(
        new ApiResponse(200, vendingRecords, "Records of previous maintenance fetched successfully. "
    ))
...
const deleteVendingMachine = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, deletedMachine, "Machine and associated products deleted successfully.")
    );
});
