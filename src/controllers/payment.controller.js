...
const initiatePayment = asyncHandler(async (req, res) => {
  ...
  const { merchant_code, pan, secret, username, password } =
    user.merchantDetails;

  // Initiate payment via QR code
  const prn = Date.now();
  const remarks1 = JSON.stringify(
    unpaidCarts.map((cart) => cart.productId.name)
  );
...
const hello = asyncHandler(async (req, res) => {
  ...
  const hell = {};
  return res.status(200).json(new ApiResponse(200, hell, "hello"));
});
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        payment,
        "Payment recorded successfully and carts updated."
      )
    );
});
